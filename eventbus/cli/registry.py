    ...
    def decorator(func: EventHandlerWithBus | EventHandlerSimple):
        ...
        if len(params) == 1:
            ...
            if not _is_event_type(hints.get(p.name)):
                raise RegistryError(f"Invalid handler for '{task_name}': single parameter must be Event.")
        ...
        # ---- Two-argument handler: must contain one Event and one Bus-like ----
            ...
            valid = (_is_bus_like_type(t1) and _is_event_type(t2)) or (_is_event_type(t1) and _is_bus_like_type(t2))

            if not valid:
                raise RegistryError(f"Invalid handler for '{task_name}': must include one Event and one BaseEventBus.")

        # ---- Invalid parameter count ----
        else:
            raise RegistryError(f"Invalid handler for '{task_name}': must have 1 or 2 parameters.")
